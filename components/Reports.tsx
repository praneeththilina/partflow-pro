...
type ReportView = 'overview' | 'revenue' | 'category' | 'customer';
        ...
        <>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <div className="bg-white p-6 rounded-3xl border border-slate-100 shadow-sm">
                    <p className="text-[10px] uppercase font-bold text-slate-400 tracking-widest mb-1">Avg. Order Value</p>
                    <p className="text-3xl font-black text-slate-800">Rs.{avgOrderValue.toLocaleString(undefined, { maximumFractionDigits: 0 })}</p>
                </div>
                <div className="bg-white p-6 rounded-3xl border border-slate-100 shadow-sm">
                            ...
                            <tbody className="divide-y divide-slate-50">
                                ...
                                {topCustomers.slice(0, 8).map((c, i) => (
                                    ...
                                    >
                                        ...
                                        <td className="py-3 text-right font-black text-slate-900 text-sm">Rs.{c.total.toLocaleString()}</td>
                                    </tr>
                                ))}
                            </tbody>
                        </table>
                ...
                <table className="w-full text-sm">
                    ...
                    <tbody className="divide-y divide-slate-50">
                        {filteredOrders.sort((a, b) => b.order_date.localeCompare(a.order_date)).map(o => (
                            <tr key={o.order_id} className="hover:bg-slate-50">
                                ...
                                <td className="px-6 py-4 text-right text-slate-500">Rs.{o.gross_total.toLocaleString()}</td>
                                <td className="px-6 py-4 text-right font-black text-slate-900">Rs.{o.net_total.toLocaleString()}</td>
                            </tr>
                        ))}
                    </tbody>
                    <tfoot className="bg-slate-900 text-white font-black">
                        <tr>
                            ...
                            <td className="px-6 py-4 text-right">Rs.{totalRevenue.toLocaleString()}</td>
                        </tr>
                    </tfoot>
                </table>
            </div>
                        ...
                        <tbody className="divide-y divide-slate-50">
                            {catLines.sort((a,b) => b.date.localeCompare(a.date)).map((l, i) => (
                                <tr key={i} className="hover:bg-slate-50">
                                    ...
                                    <td className="px-6 py-4 text-right">Rs.{l.unit_value.toLocaleString()}</td>
                                    <td className="px-6 py-4 text-right font-black">Rs.{l.line_total.toLocaleString()}</td>
                                </tr>
                            ))}
                        </tbody>
                        <tfoot className="bg-slate-900 text-white font-black">
                            <tr>
                                ...
                                <td className="px-6 py-4 text-right">Rs.{categorySales[selectedId || ''].total.toLocaleString()}</td>
                            </tr>
                        </tfoot>
                    </table>
                </div>
    ...
    return (
        <div className="space-y-6 pb-20 md:pb-0 px-2 max-w-6xl mx-auto">
                ...
                <div className="flex items-center gap-3">
                    ...
                    <h2 className="text-2xl font-black text-slate-800">
                        {view === 'overview' ? 'Business Intelligence' : 
                         ...
                         view === 'category' ? 'Category Drill-down' : 'Shop History'}
                    </h2>
                </div>
            ...
            <div id="report-content" className="bg-white p-6 md:p-10 rounded-3xl border border-slate-100 shadow-sm text-slate-900">
                {/* PDF Header (Hidden on screen, but will be shown via onclone in pdfService) */}
