...
def get_sheets_service():
    # 1. Try to load from Environment Variable (Professional Cloud Method)
    ...
    if not config:
        if not os.path.exists(SERVICE_ACCOUNT_FILE):
            print(f"CRITICAL: Service account credentials not found in Environment or File ({SERVICE_ACCOUNT_FILE})")
...
def health():
    ...
    if config:
        ...
        try:
            ...
            diag["signing_test"] = "passed"
        except Exception as sign_err:
            diag["signing_test"] = "failed"
            diag["signing_error"] = str(sign_err)
        ...
        if len(rows) > 1:
            for row in rows[1:]:
                ...
                pulled_items.append({
                    ...
                    "category": str(row[6] or 'Uncategorized'), "unit_value": unit_val, "current_stock_qty": stock_qty,
